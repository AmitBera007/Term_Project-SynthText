...
from scipy.signal import decimate, butter, lfilter, wiener
...
import librosa  # Make sure librosa is used correctly with version 0.10.0.post2
...
np.random.seed(123)
rn.seed(123)
# torch.manual_seed(999)  # Uncomment if needed for torch>=2.0.1
#-----------------------------------------#

# Paths and parameters
parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument("-i", "--input-folder", type=str, help='Path to folder that contains video files')
parser.add_argument("-a", "--audio-file", type=str, help='Path to audio file (.mp3 format)')
...
predictor_path = args.pred_path  # Ensure this path is being used correctly
...
class Extractor():
    def __init__(self, in_path, audio_file, mean_shape):
        self.dataQueue = Manager().Queue()
        self.fileList = []
        self.mean_shape = mean_shape
        self.audio_data, self.sr = librosa.load(audio_file, sr=fs)

        for root, dirnames, filenames in os.walk(in_path):
            for filename in filenames:
                if os.path.splitext(filename)[1] in ['.mp4', '.mpg', '.mov', '.flv']:
    ...
    def processSample(self, process_id):
        ...
        for j in tqdm(sampleList): 
            ...
            # Here we use the loaded audio data directly
            speech = self.audio_data
            ...
            try:
                ...
                continue
            if args.debug:
                utils.write_video_cv(np.array(frame_list), speech, 8000, out_path, os.path.join(os.path.splitext(filename)[0] + '.mp4'), 60.0)
...
if __name__ == '__main__':
    ...
    ext = Extractor(in_path, audio_file, mean_shape)
