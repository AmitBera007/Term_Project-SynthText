...
def train():
    ...
    # Debug: Check if folders exist and contain data
    print(f"Train folder: {args.in_path}")
    print(f"Validation folder: {args.val_path}")
    print(f"Files in train folder: {os.listdir(args.in_path)}")
    print(f"Files in validation folder: {os.listdir(args.val_path)}")

    dsetContainer = datagen.DatasetContainer(args)
    trainDset = dsetContainer.getDset() 

    print(f"Train dataset length: {len(trainDset)}")  # Debugging
    ...
    print(f"Validation dataset length: {len(valDset)}")  # Debugging
    ...
    # Load models if pre-trained paths are provided
    if args.model:
